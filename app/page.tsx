  ...
  return <div className="module-page">
    ...
    <section className="proof-layout">
      ...
      <aside className="proof-side">
        ...
        <article className="workspace-card evidence-gallery"><div className="card-title"><div><span className="eyebrow">LATEST CAPTURES</span><h2>Evidence feed</h2></div><button onClick={() => downloadText(`${activeProject.ref}-evidence.json`, JSON.stringify(records, null, 2))} aria-label="Export evidence register"><Download size={14} /></button></div><div className="evidence-thumbs">{records.slice(-3).map((record, index) => <button className={`thumb ${["roof","inverter","battery"][index % 3]}`} key={record.id} onClick={() => record.fileKey && window.open(`/api/files?key=${encodeURIComponent(record.fileKey)}`, "_blank")}><i>{new Date(record.capturedAt).toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit" })}</i><b>{record.category}</b></button>)}<button onClick={() => requestCapture()}><Plus size={18} /><small>Add capture</small></button></div></article>
      </aside>
    </section>
  </div>;
}
